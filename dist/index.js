...
(() => {
  ...
  // src/animations/mobile-menu-animation.ts
  var MobileMenuAnimation = class {
    textSplit;
    tl;
    NAVIGATION_LINKS_WRAPPER = '[navigation="links-wrapper"]';
    navigationLinksWrapper;
    NAVIGATION_LINK = '[navigation="link"]';
    navigationLinks;
    NAVIGATION_LOGO = '[navigation="logo"]';
    logo;
    NAVIGATION_MENU_BUTTON = '[navigation="menu-button"]';
    menuButton;
    _isMenuOpen = false;
    get isMenuOpen() {
  ...
  // src/animations/navigation-transition.ts
  var NavigationTransitionAnimation = class {
    PAGE_TRANSITION_WRAPPER = '[page-transition="wrapper"]';
    PAGE_TRANSITION_LOGO = '[page-transition="logo"]';
    /** Only here for documentation purposes to show which attribute to set to disable the page transition animation. */
    ANCHOR_NO_PAGE_TRANSITION = '[page-transition="disable"]';
    _anchorElements;
    tl;
    /**
    ...
    constructor() {
      ...
      this.tl = gsapWithCSS.timeline({ paused: true, defaults: { ease: "expo" } });
      this.tl.add("start");
      this.tl.fromTo(
     ...
     */
    async playPageTransition(direction) {
  ...
  // src/animations/base/scroll-animation.ts
  var ScrollAnimation = class {
     ...
     */
    dispose() {
      this._timelines.forEach((timeline2) => {
        timeline2.kill();
      });
      this._timelines = [];
  ...
  // src/utils/gsap-attributes.ts
  var ScrollTriggerAttributes = class {
    id;
    begin;
    end;
    direction;
     ...
     */
    constructor(element) {
      this.id = (element.getAttribute("scroll-trigger-id") || element.getAttribute("id")) ?? void 0;
      this.begin = element.getAttribute("begin") ?? void 0;
      this.end = element.getAttribute("end") ?? void 0;
      this.direction = element.getAttribute("direction") ?? "forward";
    }
  };
  var TimelineAttributes = class {
    ...
    constructor(element) {
      ...
      this.ease = _ease ?? void 0;
    }
  };

  // src/animations/fade.ts
  var FadeInScrollAnimation = class extends ScrollAnimation {
    /**
    ...
    initialize() {
      this._elements.forEach((element, i) => {
        ...
        const tl = gsapWithCSS.timeline({ paused: true, delay: tlAttributes.delay });
        ...
        this._scrollTriggers.push(
          ScrollTrigger2.create({
            ...
            markers: false,
            onEnter: () => tl.play(),
            onLeave: stAttributes.direction == "both" ? () => tl.reverse() : void 0,
            onEnterBack: stAttributes.direction == "both" ? () => tl.play() : void 0,
            onLeaveBack: stAttributes.direction == "both" ? () => tl.reverse() : void 0
          })
        );
      });
    }
  ...
  var TextSplit = class extends EventTarget {
    _windowWidth;
    _splitType;
    _options;
    /**
     ...
     */
    constructor(target = "[text-split]", options = {
      types: ["lines", "words", "chars"],
      tagName: "span",
      lineClass: "text-line"
    }, debounceTimeout = 100) {
  ...
  var matchPhone = window.matchMedia("(max-width: 479px)");
  var matchPhoneLandscape = window.matchMedia("(min-width: 480px and max-width: 767px)");
  var matchPhoneAll = window.matchMedia("(max-width: 767px)");
  var matchTablet = window.matchMedia("(min-width: 768px and max-width: 991px)");
  var matchMobileAll = window.matchMedia("(max-width: 991px)");
  var matchDesktop = window.matchMedia("(min-width: 992px)");
  var isMobileDevice = () => {
    if ("ontouchstart" in document.documentElement) {
      return true;
    }
  };
  var matchOrientationLandscape = window.matchMedia("(orientation: landscape)");

  // src/utils/rem-2-px.ts
  var rem2Px = (rem) => {
  ...
  var TextSplitScrollAnimation = class extends ScrollAnimation {
    _scope;
    _textSplit;
    get textSplit() {
     ...
     */
    constructor(textElementSelector, scope) {
    ...
    textSplitReloadHandler() {
      this.reload(false);
    }
    /**
     * Reloads the animation.
     */
    reload(reloadTextSplit = true) {
      this._timelines = [];
      this._scrollTriggers = [];
      if (reloadTextSplit) {
  ...
  var TextSlideUpScrollAnimation = class extends TextSplitScrollAnimation {
    /**
    ...
    initialize() {
      this._elements.forEach((element, i) => {
        const stAttributes = new ScrollTriggerAttributes(element);
        const tlAttributes = new TimelineAttributes(element);
        gsapWithCSS.set(element.querySelectorAll(".text-line"), { overflow: "hidden" });
        const tl = gsapWithCSS.timeline({ paused: true, delay: tlAttributes.delay });
        this._timelines.push(tl);
  ...
  var CharsSlideUpScrollAnimation = class extends TextSlideUpScrollAnimation {
    /**
  ...
  var WordsSlideUpScrollAnimation = class extends TextSlideUpScrollAnimation {
    /**
  ...
  var TextFadeInScrollAnimation = class extends TextSplitScrollAnimation {
    /**
  ...
  var CharsFadeInScrollAnimation = class extends TextFadeInScrollAnimation {
    /**
  ...
  var WordsFadeInScrollAnimation = class extends TextFadeInScrollAnimation {
    /**
  ...
  // src/interactions/horizontal-slideshow.ts
  var HorizontalSlideshow = class {
    LIST_ATTRIBUTE = "slideshow-list";
    LIST_ITEM_ATTRIBUTE = "slideshow-item";
    BUTTON_RIGHT_ATTRIBUTE = "slideshow-button-right";
    BUTTON_LEFT_ATTRIBUTE = "slideshow-button-left";
    _scrollTween;
    _list;
    _listItem;
    _gap;
    ...
    initialize() {
      ...
      gsapWithCSS.set(this._buttonLeft, { opacity: 0.5, cursor: "not-allowed" });
      window.addEventListener(
        "resize",
        debounce(() => {
          this.reload();
    ...
    buttonLeftClickHandler() {
      if (Math.floor(this._maxRightScroll) > 0) {
        ...
        gsapWithCSS.set(this._buttonRight, { opacity: 1, cursor: "pointer" });
        if (Math.floor(this._maxRightScroll) == 0) {
          gsapWithCSS.set(this._buttonLeft, { opacity: 0.5, cursor: "not-allowed" });
        }
      }
    ...
    buttonRightClickHandler() {
      if (Math.floor(this._maxLeftScroll) > 0) {
        ...
        this._maxLeftScroll -= 1;
        this._maxRightScroll += 1;
        gsapWithCSS.set(this._buttonLeft, { opacity: 1, cursor: "pointer" });
        if (Math.floor(this._maxLeftScroll) == 0) {
          gsapWithCSS.set(this._buttonRight, { opacity: 0.5, cursor: "not-allowed" });
        }
      }
    }
    scroll(direction) {
      ...
      this._scrollTween = gsapWithCSS.to(this._list, {
  ...
  var Cursor = class {
    ...
    constructor() {
      if (matchDesktop.matches && !isMobileDevice()) {
        document.querySelectorAll("a, button, [cursor-target]").forEach((element) => {
  ...
  var Collapsible = class {
     ...
     */
    constructor(collapsibleDisplayMode, iconOptions = { initialRotation: 0, expandedRotation: 180 }) {
      const clickTarget = document.querySelector(this.CLICK_TARGET_SELECTOR);
      const icon = document.querySelector(this.ICON_SELECTOR);
      const collapsible = document.querySelector(this.COLLAPSIBLE_SELECTOR);
      console.log(collapsible?.offsetHeight);
      const tl = gsapWithCSS.timeline({ paused: true, defaults: { ease: "sine.inOut" } });
      tl.add("start");
      tl.fromTo(
        collapsible,
  ...
  var Page = class _Page {
    // Animations
    _mobileNavigationAnimation;
    _navigationTransitionAnimation;
    _charsSlideUpScrollAnimation;
    _wordsSlideUpScrollAnimation;
    _fadeInScrollAnimation;
    _charsStaggeredFadeAnimation;
    _wordsStaggeredFadeAnimation;
    // Interactions
    _cursorInteraction;
    // Properties
    _isLoaded = false;
    _isPerformantDevice = navigator.hardwareConcurrency > 2;
    /**
    ...
    constructor() {
      ...
      if (this._isPerformantDevice) {
        ...
        this._charsStaggeredFadeAnimation = new CharsFadeInScrollAnimation();
        this._wordsStaggeredFadeAnimation = new WordsFadeInScrollAnimation();
      }
      this._navigationTransitionAnimation = new NavigationTransitionAnimation();
      if (this.constructor === _Page) {
        this.finishLoading();
      }
    }
    /**
  ...
  var DienstenPage = class extends Page {
    /**
    ...
    constructor() {
      ...
      ScrollTrigger2.getAll().forEach((st) => st.refresh());
      this.finishLoading();
    }
  };

  // src/pages/projecten.ts
  var ProjectenPage = class extends Page {
    /**
     * Create a new Diensten page.
     */
    constructor() {
